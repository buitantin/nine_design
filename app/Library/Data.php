...
class Data
{
	...
	public function filterData($string) {
		...
		$string = Data::inject($string);
		$string = Data::htmlDecode($string);
		return $string;
	}
	public function asignData($param)
	{
		...
		foreach ($param as $key => $value) {
			if(is_string($value))
			$data[$key] = Data::filterData($value);
			else 
			{
				$data[$key] = $value;
			}
	...
	{
		return Data::filterData(Data::htmlEncode(Data::stripTags($field)));
	}
	public function tranferData($string)
	{
		$trans = array(
  		...
  		$string = strtr(Data::htmlDecode($string), $trans);
  		$string = Data::filterData($string);
	...
	public function toAlias($string)
	{
		...
		$string = Data::tranferData($string);
		...
		return Data::toLower(substr($string, 1));
	}
	...
	public static  function toAlias2($string)
	{
		...
		$string = Data::tranferData2($string);
		$string = strip_tags($string);
		$string = trim($string, " \n\t.");
		$string = str_replace($tmp,"",$string);
		...
		return Data::toLower2(substr($string, 1));
	}
	   ...
	   while($file = readdir($dir_handle)) {
	      if ($file != "." && $file != "..") {
	         if (!is_dir($dirname."/".$file))
	            ...
	            Data::delete_directory($dirname.'/'.$file);    
	      }
	   }
	   closedir($dir_handle);
	   rmdir($dirname);
	...
	public static function recurse_copy($src,$dst,$id=1,$id_old=1) {
		...
		while(false !== ( $file = readdir($dir)) ) {
			if (( $file != '.' ) && ( $file != '..' )) {
				if ( is_dir($src . '/' . $file) ) {
					Data::recurse_copy($src . '/' . $file,$dst . '/' .$file ,$id, $id_old);
				}
				else {
	...
	public static function randd($option=12){
		 $int = rand(0,10);
	    $a_z = "01234567891";
	    $rand_letter = $a_z[$int];
	    for($i=0;$i<$option;$i++){
	...
	public static  function uploadProduct($source,$filename){
		move_uploaded_file($source, PRODUCTS_PATH."big/".$filename);

		Image::make(PRODUCTS_PATH."big/".$filename,array(
			    'width' => 220,
			    'height' => 220
			
			))->save(PRODUCTS_PATH."small/".$filename);
		
		return true;
		
	}
	public static  function uploadBlog($source,$filename){
		move_uploaded_file($source, BLOG_PATH."big/".$filename);

		Image::make(BLOG_PATH."big/".$filename,array(
			    'width' => 220,
			    'height' => 220
			
			))->save(BLOG_PATH."small/".$filename);
	...
	public static function remove_all_ImageProduct_New($id_product){
		Data::delete_directory(PRODUCTS_PATH."product".$id_product);
	}
...
// 			return $cached_src_one;
    ...
    public static function uploadbank_promotion($source,$id_product,$filename){
		...
		Data::createFolder($pathIn."bankpro".$id_product);
